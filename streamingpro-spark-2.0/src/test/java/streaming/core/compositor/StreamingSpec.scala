...
import streaming.core.strategy.platform.{SparkStreamingRuntime, SparkStructuredStreamingRuntime}
...
class StreamingSpec extends BasicStreamingOperation {
  ...
  "streaming with join table" should "run normally" in {
    ...
    withStreamingContext(setupStreamingContext(streamingParams, "classpath:///test/ss-test.json")) { runtime: SparkStructuredStreamingRuntime =>
      val batchCounter = new BatchCounter(runtime.)
