...
/**
  ...
  */
abstract class BaseTransformCompositor[T, S: ClassTag, U: ClassTag] extends Compositor[T] {

  protected var _configParams: util.List[util.Map[Any, Any]] = _

  val logger = Logger.getLogger(classOf[BaseTransformCompositor[T, S, U]].getName)
  ...
  override def result(alg: util.List[Processor[T]], ref: util.List[Strategy[T]], middleResult: util.List[T], params: util.Map[Any, Any]): util.List[T] = {
    ...
    val func = transform
    val newDstream = dstream.transform(line => func(line))
    List(newDstream.asInstanceOf[T])
  }

  def transform: RDD[S] => RDD[U]
}
