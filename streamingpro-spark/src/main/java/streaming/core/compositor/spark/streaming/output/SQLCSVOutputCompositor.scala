 ...
 */
class SQLCSVOutputCompositor[T] extends Compositor[T] with CompositorHelper with ParamsValidator {

  private var _configParams: util.List[util.Map[Any, Any]] = _
  val logger = Logger.getLogger(classOf[SQLCSVOutputCompositor[T]].getName)
  ...
  override def result(alg: util.List[Processor[T]], ref: util.List[Strategy[T]], middleResult: util.List[T], params: util.Map[Any, Any]): util.List[T] = {
    ...
    dstream.foreachRDD { rdd =>
      try {
        val df = func(rdd)
        df.write.options(_cfg).mode(SaveMode.valueOf(_mode)).format("com.databricks.spark.csv").save(_resource)
      } catch {
  ...
  override def valid(params: util.Map[Any, Any]): (Boolean, String) = {
    if (path.isDefined) (true, "")
      ...
      (false, s"Job name = ${params("_client_")}, Compositor=SQLCSVOutputCompositor,Message = path required")
  }
}
