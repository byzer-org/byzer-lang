...
import org.apache.spark.sql.catalyst.plans.logical.{Aggregate, Filter, GlobalLimit, LocalLimit, LogicalPlan, Project, Sort, Subquery, SubqueryAlias}
...
import tech.mlsql.indexer.impl.KylinSQLDialect
...
case class KylinPushdownSourceInfo(props: Map[String, String], sparkSession: SparkSession, lr:LogicalRelation) extends PushdownSourceInfo(props) with Pushdownable {
  ...
  override def fastEquals(other: PushdownSourceInfo): Boolean = {
    other match {
      case mysql: KylinPushdownSourceInfo =>
  ...
  override def buildScan(plan: LogicalPlan, sparkSession: SparkSession): DataFrame = {
    val sqlBuilder = new LogicalPlanSQL(plan, new KylinSQLDialect)
  ...
  override def buildScan(lp: LogicalPlan): DataFrame = {
    buildScan(lp, sparkSession)
  }
  ...
  override def buildScan2(lp: LogicalPlan, sparkSession: SparkSession): LogicalPlan = {
    ...
    val sqlBuilder = new LogicalPlanSQL(newlp, new KylinSQLDialect)
    val sql = sqlBuilder.toSQL
    var newsub = newlp
    lp match {
  ...
  override def buildScan2(lp: LogicalPlan): LogicalPlan = {
    buildScan2(lp, sparkSession)
  }
}
