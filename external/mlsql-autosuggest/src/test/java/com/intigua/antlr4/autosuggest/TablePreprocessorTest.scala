...
class TablePreprocessorTest extends BaseTest {
  ...
  test("load/select table") {
    ...
    assert(targetTable3 == MetaTable(MetaTableKey(None, Some(SpecialTableConst.TEMP_TABLE_DB_KEY), "table3"), List(MetaTableColumn("a", null, true, Map()), MetaTableColumn("b", null, true, Map()), MetaTableColumn("c", null, true, Map()))))
  }
