...
object CipherUtils extends Logging with WowLog {
  ...
  private def getKeyAndIvWithDefault(key: String, iv: String): (Array[Byte], IvParameterSpec) = {
    ...
    if (key != null) {
      if (key.length != 16) {
        logDebug("The size of BYZER_CIPHER_AES_KEY is not 128 bit, it is simplified by default!")
    ...
    if (iv != null) {
      if (iv.length != 16) {
        logDebug("The size of BYZER_CIPHER_AES_IV is not 128 bit, it is simplified by default!")
